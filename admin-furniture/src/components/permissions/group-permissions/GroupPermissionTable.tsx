...
import GroupPermissionTableBody from "./GroupPermissionTableBody";
import GroupPermissionTableHead from "./GroupPermissionTableHead";
import GroupPermissionTableToolbar from "./GroupPermissionTableToolbar";

export type Order = "asc" | "desc";

export default function GroupPermissionTable() {
  const [selected, setSelected] = useState<readonly string[]>([]);
  const [options, setOptions] = useQueryParams<ListOptions<GroupPermission>>({
    limit: 10,
    offset: 0,
    sortBy: "name",
    sortOrder: "asc",
  ...
  const { data, error, isLoading, isValidating } = useGroupPermissionList(options);
  ...
  const handleRequestSort = (
    ...
    property: keyof GroupPermission,
  ) => {
